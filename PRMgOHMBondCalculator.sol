...
contract PRMgOHMBondCalculator is IBondingCalculator {
    ...
    constructor( address _PRM, address _hecDaiPair ) {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == PRM ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( PRM ).decimals() ) ).div( getTotalValue( _pair ) );
    }
