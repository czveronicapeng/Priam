...
contract PriamBondStakeDepository is Ownable {
    ...
    address public immutable PRM; // intermediate reward token from treasury
    address public immutable sPRM; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints PRM when receives principle
    ...
    struct Bond {
        uint gonsPayout; // sPRM gons remaining to be paid
    ...
    constructor ( 
        ...
        require( _PRM != address(0) );
        PRM = _PRM;
        require( _sPRM != address(0) );
        sPRM = _sPRM;
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 PRM ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) PRM
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( PRM ).safeTransfer( DAO, fee ); 
        }
        ...
        IERC20( PRM ).approve( stakingManager, payout );

        IStakingManager(stakingManager).stake( payout, address(this) );
        /* ---------------------------------------------------------- */
        
        uint stakeGons=ISPRM(sPRM).gonsForBalance(payout);
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISPRM(sPRM).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sPRM ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( PRM ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice return bond info with latest sPRM balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISPRM(sPRM).balanceForGons(info.gonsPayout);
    ...
    /**
     *  @notice calculate current ratio of debt to PRM supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( PRM ).totalSupply();
    ...
    /**
     *  @notice calculate amount of PRM available for claim by depositor
     *  @param _depositor address
     *  @return pendingPayout_ uint
     */
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _depositor );
        uint payout = ISPRM(sPRM).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or PRM) to the DAO
     *  @return bool
     */
