...
contract PriamBondDepositoryV2 is Ownable {
    ...
    address public immutable PRM; // token given as payment for bond
    ...
    struct Bond {
        uint payout; // PRM remaining to be paid
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( PRM ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( PRM ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( PRM ).approve( staking, _amount );
