...
contract PriamTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SPRM }

    address public immutable PRM;
    ...
    address public sPRM;
    uint public sPRMQueue; // Delays change to sPRM address
    ...
    /**
        @notice allow approved address to deposit an asset for PRM
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint PRM needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( PRM ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn PRM for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IPRMERC20( PRM ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sPRM ).balanceOf( msg.sender ); // Can only borrow against sPRM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with PRM
        @param _amount uint
     */
    function repayDebtWithPRM( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IPRMERC20( PRM ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, PRM, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( PRM ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( PRM ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns PRM valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match PRM decimals
            value_ = _amount.mul( 10 ** IERC20( PRM ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SPRM ) { // 9
            sPRMQueue = block.number.add( blocksNeededForQueue );
        } else return false;
